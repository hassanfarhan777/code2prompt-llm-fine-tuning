...
class tools_local():
    ...
    def extract_quoted_text(self, raw_gpt_response, verbose=True):
        """
        Extract text between quotes from a list of strings.
        
        Args:
            raw_gpt_response (list): List of strings to extract quoted text from
            verbose (bool): Whether to print extraction details
        
        Returns:
            list: List of extracted quoted texts
        """
        all_extracted_texts = []
        
        for i in range(len(raw_gpt_response)):
            # Extract text between double or single quotes
            # Using regex to find all quoted strings, including across multiple lines (re.DOTALL flag)
            matches = re.findall(r'"(.*?)"|\'(.*?)\'', raw_gpt_response[i], re.DOTALL)
            
            # Flatten the list of tuples and filter out empty strings
            flattened_matches = [match[0] or match[1] for match in matches if any(match)]
            
            # Join the extracted parts with a space (or you can choose a different separator)
            ...
            if verbose:
                ...
                # Use self.printer if it exists, otherwise use standard print
                if hasattr(self, 'printer') and callable(self.printer):
                    self.printer(all_extracted_texts[i])
                else:
                    print(all_extracted_texts[i])
        
        # Print first extracted text for verification if verbose and texts exist
        if verbose and all_extracted_texts:
            print("\n--- First Extracted Prompt (for verification) ---")
            print(all_extracted_texts[0])
            print("--------------------------------------------------")
...
class llm_tools():
    ...
    def __init__(self, api_key=None):
        """
        Initialize the llm_tools class with OpenAI client.
        
        Args:
            api_key (str, optional): OpenAI API key. If not provided, 
                                   it will use the globally set openai.api_key
        """
        if api_key:
            self.client = openai.OpenAI(api_key=api_key)
        else:
            # Use the globally set API key (as you're doing in your notebook)
            self.client = openai.OpenAI()
    
    def prompt_generator(self, formatted_code_string, model="gpt-4o-mini", max_tokens=400, temperature=0.01):
        """
        Prepares messages for the Chat API and sends a request to generate a natural language prompt
        that would produce the given code snippet.
        ...
        Args:
            ...
            max_tokens (int): The maximum number of tokens in the response. Defaults to 400.
        ...
        messages = [
            {
                "role": "user", 
                "content": f"You are a helpful assistant which would suggest a prompt from the user which would generate such a code: {formatted_code_string}. Make sure the language of the prompt is very natural"
            }
        ]
    ...
    def generate_and_extract(self, file_contents):
        """
        ...
        for i, content in enumerate(file_contents):
            print(f"\nProcessing code {i}:")
            response = self.prompt_generator(content)
            
            # Handle case where response is None (API error)
            if response is None:
                print("Skipping due to API error...")
                all_extracted_texts.append("")  # Add empty string to maintain list alignment
                continue
                
            response_text = response.choices[0].message.content
            # self.printer(response_text) if hasattr(self, 'printer') else None  # Optional: print each response
